...
class DataCollector:
    ...
    def collect_all_reference_data(self) -> Dict[str, Any]:
        ...
        template_vars = {"request_limit": self._batch_limit}
        endpoints = self._config_manager.load_endpoints(template_vars)
    ...
    def _collect_endpoint_data(self, endpoint_config: Dict[str, Any]) -> tuple[List[Dict[str, Any]], Dict[str, Any]]:
        ...
        count_endpoint_url = properties['count_endpoint_url']
        http_method = properties.get('http_method', 'POST')
        ...
        try:
            ...
            collected_data, batches_used = self._query_with_batching(
                endpoint_url=endpoint_url,
                count_endpoint_url=count_endpoint_url,
    ...
    def _query_count(self, count_endpoint_url: str, body: str = "") -> int:
        ...
        try:
            ...
            elif isinstance(response, list) and len(response) > 0 and 'count' in response[0]:
                return response[0]['count']
            else:
                raise ValueError(f"Unexpected response format from count endpoint: {response}")
                
        except Exception as e:
            raise Exception(f"Failed to query count for endpoint {count_endpoint_url}: {str(e)}")
    ...
    def _query_data(
        ...
        try:
            ...
            raise Exception(f"Failed to query endpoint {endpoint_url}: {str(e)}")
    
    def _query_with_batching(
        self,
        endpoint_url: str,
        count_endpoint_url: str,
        body_template: str,
        batch_limit: int,
        http_method: str = "POST"
        ...
        if total_count <= batch_limit:
            ...
            body = body_template.replace("{{request_limit}}", str(batch_limit))
            records = self._query_data(endpoint_url, body, http_method)
            ...
            while offset < total_count:
                ...
                # Build body with limit and offset
                body = body_template.replace("{{request_limit}}", str(current_limit))
                
                # Insert offset after the limit statement
                # Find the limit statement and add offset after it
