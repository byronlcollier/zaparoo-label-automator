...
class LabelGenerator:
    ...
    def _find_platform_logo(self, platform_info, platform_folder):
        """
        Find the platform logo file path.
        ...
        """
        # Look for platform logo in versions
        versions = platform_info.get('versions', [])
        for version in versions:
            platform_logo = version.get('platform_logo', {})
            local_file_path = platform_logo.get('local_file_path')
            if local_file_path:
                logo_path = platform_folder / local_file_path
                if logo_path.exists():
                    return logo_path
